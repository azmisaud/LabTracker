...
$(document).ready(function() {
    // Remove the toggle functionality since form is always visible
    // $('#update-data-btn').on('click', function() {
    //     $('#update-data-form').toggle();
    //     $('#update-data-btn').hide();
    //     $('#start-new-semester-btn').hide();
    ...
    $('#update-data-form select:first').change(function() {
        const course = $(this).val();
        $.ajax({
            url: '/faculty/get-semesters/',
            data: { course: course },
            success: function(data){
                const semesterSelect = $('#update-data-form select:last');
    ...
    $('#update-data-form').on('submit', function(event) {
        event.preventDefault();
        const course = $(this).find('select:first').val();
        const semester = $(this).find('select:last').val();
        const submitButton = $(this).find('button[type="submit"]');

        // Add loading state
        submitButton.addClass('loading');
        submitButton.html('<span class="loading-icon">Updating...</span>');
        ...
        $.ajax({
            ...
            complete: function() {
                submitButton.removeClass('loading');
                submitButton.html('Update Data');
            }
        });
    });

    // Start new semester handler
    $('#start-new-semester-btn').click(function() {
        if (confirm('Are you sure you want to start a new semester? This will delete students who have been on the platform for more than or equal to 150 days.')) {
            const button = $(this);
            button.addClass('loading');
            button.html('<span class="loading-icon">Processing...</span>');
            ...
            $.ajax({
                ...
                complete: function() {
                    ...
                    button.html('Start New Semester');
                }
            });
        }
    });
